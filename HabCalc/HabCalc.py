...
dams_outs=np.concatenate((dams,outs))
...
locations1=np.genfromtxt(f_dams,delimiter=',',skip_header=1,usecols=(3,4,5,6,7),dtype='str')
locations2=np.genfromtxt(f_outlets,delimiter=',',skip_header=1,usecols=(3,4,5,6,7),dtype='str')
...
termcode=np.genfromtxt(f_chandat,dtype='str',delimiter=',',skip_header=1,usecols=(5))
flow=np.loadtxt(f_lines,delimiter=',',skiprows=1,usecols=(0,1))

#get COMIDS of everything within place requested by user
#import pdb; pdb.set_trace()
comids=chandat[np.char.find(termcode,place)==0,0]
#keep only flowlines/features that match the comids list selected by location
...
damOrder = np.array([-1] * dams_outs.size)
#pdb.set_trace()
...
for i in xrange(dams_outs.size):
    ...
    print '%i of %i' % (i+1, len(dams_outs)) #show them you're working on it
    ...
    while np.sum(current)>0: #break this while loop if there are no more upstream flowlines
        ...
        if np.any(np.isin(dams_outs,current)):
            #if np.isin(IDs[i],IDs[np.isin(dams_outs,current,invert=True)]):
            #    pdb.set_trace()
            current_passageComp=passageComp[i]
            passageComp[(np.isin(dams_outs,current))] = passageComp[(np.isin(dams_outs,current))] * passage[i]
            ...
            damOrder[(np.isin(dams_outs,current))] += 1
...
f.write('UNIQUE_ID,type,catchmentID,habitat_sqkm,habitatSegment_sqkm,functional_habitatSegment_sqkm,PassageToHabitat,Decision,TERMCODE,terminal_name_huc10,terminal_name_huc8,terminal_name_huc6,terminal_name_huc4,dam_name,dam_order\n')
for i in idx:
    f.write('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % (IDs[i], type[i], dams_outs[i], habup[i], habseg[i], habseg[i]*passageComp[i], passageComp[i], decision[i],locations[i,0],locations[i,1],locations[i,2],locations[i,3],locations[i,4],'foo',damOrder[i]))
f.close()
